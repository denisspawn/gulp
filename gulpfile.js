...
gulp.task('sass', function() {
	return gulp.src('app/sass/**/*.sass') // шаблоны выборки могут быть разными. Если перд выборкой поставить "!" то этот файл попадет в исключения и не попадет в выборку.
...
gulp.task('browser-sync', function() {
	browserSync.init({ // Выполняем browserSync
		proxy: 'gulp.pro', // для того, чтобы это заработало и работали php файлы нужно настроить виртуальные хосты в apache
...
gulp.task('watch', function() {
	gulp.watch('app/sass/**/*.sass', gulp.series('sass')); // series выполняет все действия последовательно (пока однис скрипт не завершит работу - второй не запустится) они не будут работать параллельно
	gulp.watch('app/libs/**/*.js', gulp.series('scripts')); // Наблюдение за JS файлами в папке js
	gulp.watch('app/**/*.+(php|html)').on('change', browserSync.reload); // Наблюдение за php|html файлами в корне проекта
});
